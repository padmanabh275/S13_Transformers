...
class ReisdualConnection(nn.Module):
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def __init__(self, d_model: int, h: int, dropout: float) -> None:
        super().__init__()
        self.d_model = d_model # Embediing vector size
...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, self_attention_block: MultiHeadAttentionBlock, feed_forward_block: FeedForwardBlock):
        ...
        self.residual_connections = nn.ModuleList([ReisdualConnection(dropout) for _ in range(2)])
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, self_attention_block: MultiHeadAttentionBlock, cross_attention_block: MultiHeadAttentionBlock, feed_forward_block: FeedForwardBlock):
        ...
        self.feed_forward_block = feed_forward_block
        self.residual_connections = nn.ModuleList([ReisdualConnection(dropout) for _ in range(3)])
...
def build_transformer(src_vocab_size:int,tgt_vocab_size:int,src_seq_len:int,tgt_seq_len:int,d_model:int=512,N: int=6,h:int=8,dropout:float=0.1,d_ff:int=1048) ->Transformer:
