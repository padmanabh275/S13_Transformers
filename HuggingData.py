...
class HuggingDataModule(LightningDataModule):
    ...
    def prepare_data(self) -> None:
        # hugging-face(OPUS BOOKS) only has train split, so we will divide it later
        load_dataset(
            "opus_books",
    ...
    def setup(self, stage: str) -> None:
        ...
        if not self.data_train and not self.data_val:
            ...
            # build tokenizer
            tokenizer_src = get_or_build_tokenizer(self.hparams.config, ds_raw, lang_src)
            tokenizer_tgt = get_or_build_tokenizer(self.hparams.config, ds_raw, lang_tgt)
